 ...
 * Handles texts longer than 1000 characters by splitting and merging
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const analysis = analyzeTextProcessing(body.text, 1000, 10000)
    ...
    // Check for timeout or processing errors
    if (errorMessage.includes('timeout') || errorMessage.includes('タイムアウト')) {
      return NextResponse.json(
        { error: 'テキストの処理がタイムアウトしました。テキストを短くして再試行してください。' },
        { status: 408 }
      )
    }
